...
// });
  ...
  CHECK(status IN ('Pending','Paid','Raised','In Process','Hold','Rejected')),
  remarks TEXT,
  due_date TEXT,
...
db.serialize(() => {
db.run(`ALTER TABLE expense_payments RENAME TO expense_payments_old`);
...
(id, expense_id, month_year, actual_amount, paid_amount, paid_date, status, remarks, due_date)
SELECT
id, expense_id, month_year, actual_amount, paid_amount, paid_date, status, remarks, due_date
FROM expense_payments_old
`);

db.run(`DROP TABLE expense_payments_old`);
});
...
function generateInvoiceNumber(db, callback) {
...
next = parseInt(row.invoice_number.split("-").pop(), 10) + 1;
}
...
(err, project) => {
if (err) {
return res.status(500).json({ error: "Project fetch failed" });
}
if (!project) {
...
// ---------- STEP 4: SAFE INSERT WITH RETRY ----------
const insertWithRetry = (attempt = 1) => {
if (attempt > 3) {
return res
.status(500)
.json({ error: "Failed to generate unique invoice number" });
}

generateInvoiceNumber(db, (err, invoice_number) => {
if (err) {
return res
.status(500)
.json({ error: "Invoice number generation failed" });
}
...
function updateSalaryOnExpensePaid(expense, expenseType, month_year, paidAmount, paidDate, dbInstance) {
...
const targetMonth = month_year.slice(0, 7); // 🔑 Use paid month

// 🔑 Use provided db instance or fallback to global db
...
dbToUse.run(updateQuery, [paidAmount, paidAmount, paidDate, employeeId, targetMonth], function (err) {
if (err) {
console.error("❌ Error running monthly salary UPDATE:", err);
return;
}
if (this.changes && this.changes > 0) {
console.log(`✅ Updated monthly_salary_payments for ${employeeId} (${targetMonth}) - Paid: ${paidAmount}`);
return;
}
...
VALUES (?, ?, ?, 'Yes', ?, ?, ?)
`;
dbToUse.run(insertQuery, [employeeId, employeeName || "", targetMonth, paidAmount, paidAmount, paidDate], function (insertErr) {
if (insertErr) {
console.error("⚠️ Insert into monthly_salary_payments failed:", insertErr);
dbToUse.run(updateQuery, [paidAmount, paidAmount, paidDate, employeeId, targetMonth], function (finalErr) {
...
console.log(`✅ Inserted monthly_salary_payments for ${employeeId} (${targetMonth}) - Paid: ${paidAmount}`);
});
});
}
...
// ----------------------------------------------------------------------
// ⭐ UPDATED → CREDIT KITTY + DEBIT INSURANCE FROM CURRENT
// ----------------------------------------------------------------------
const continueToKittyCredit = () => {
db.get(
`SELECT employee_name, total_insurance FROM salary_payments WHERE employee_id = ? LIMIT 1`,
...
// =====================================================
// 🔥 NEW → Debit Insurance From CURRENT First
// =====================================================
db.get(
`SELECT * FROM accounts WHERE account_type = 'Current' LIMIT 1`,
[],
...
res.status(500).json({
success: false,
message: "Current account not found for insurance debit",
})
);
}

const currPrevBal = parseFloat(currentAcc.balance);
const currNewBal = currPrevBal - insuranceAmount;

if (currNewBal < 0) {
return db.run("ROLLBACK", () =>
res.status(400).json({
success: false,
message: "Insufficient CURRENT balance for insurance debit",
})
);
}
...
db.run(
`UPDATE accounts SET balance = ? WHERE account_id = ?`,
[currNewBal, currentAcc.account_id],
function (currUpdErr) {
if (currUpdErr) {
return db.run("ROLLBACK", () =>
res.status(500).json({
success: false,
message: "Insurance debit CURRENT update failed",
})
);
}

// Record insurance debit transaction
const now = new Date();
const hh = now.getHours().toString().padStart(2, "0");
const mm = now.getMinutes().toString().padStart(2, "0");
const hhmm = `${hh}${mm}`;

const insTxId = `DEB|Insurance|Salary|${month}|${hhmm}`;
...
currentAcc.account_number,
insuranceAmount,
`Insurance debit for ${employeeName} (${employeeId})`,
currPrevBal,
currNewBal,
],
(txErr) => {
...
res.status(500).json({
success: false,
message: "Insurance debit transaction failed",
})
);
}

// =====================================================
// ⭐ EXISTING KITTY CREDIT (UNCHANGED)
// =====================================================
const prevBal = parseFloat(kittyAcc.balance);
const newBal = prevBal + insuranceAmount;
...
kittyAcc.account_number,
insuranceAmount,
`Insurance credit for ${employeeId}`,
prevBal,
newBal,
],
(txErr2) => {
...
// Salary debit function unchanged
function debitCurrentAccount(employeeName) {
db.get(
`SELECT * FROM accounts WHERE account_type = 'Current' LIMIT 1`,
[],
...
currentAcc.account_number,
paidAmtVal,
`Salary debit for ${employeeName} (${employeeId})`,
prevBal,
newBal,
],
(txErr) => {
...
// const { expense_id, payment_row_id, paid_amount, paid_date } = req.body;
...
// // 🔑 ONLY FIX: make transaction_id unique
...
// const transactionId =
// `DEB|${isRegular}|${expenseType}|${paidMonthYear}|${timeCode}${seconds}|${rand}`;

// const description = `Expense paid for ${expenseType} of month ${paidMonthYear}`;

// // 2️⃣ Fetch Current account
...
// return res.status(400).json({
// success: false,
// message: `Insufficient funds. Balance ₹${prevBalance}, required ₹${amount}`,
// });
// }
...
// if (txErr) {
// return db.run("ROLLBACK", () =>
// res.status(500).json({ success: false, message: "Transaction insert failed" })
// );
// }
...
//     // ✅ defer salary / insurance / kitty updates
//     setTimeout(() => {
//       try {
//         updateSalaryOnExpensePaid(
//           expense,
//           expenseType,
//           paidMonthYear,
//           amount,
...
// // ✅ CASE A: Pay existing DB row
// if (payment_row_id) {
// db.run(
// `UPDATE expense_payments
// SET paid_amount = ?, paid_date = ?, status = 'Paid'
// WHERE id = ?`,
// [amount, paid_date, payment_row_id],
// function (updErr) {
...
// if (insErr) {
// return db.run("ROLLBACK", () =>
// res.status(500).json({ success: false, message: "Payment insert failed" })
// );
// }
// commitAndRespond();
// }
...
/* =========================
HELPER: Handle Insurance Debit/Credit for Salary Expenses
========================= */
function handleInsuranceForSalaryExpense(expense, expenseType, paidAmount, paidDate, currentAcc, prevBalance, newBalance, callback) {
  // Check if this is a salary expense
  if (!/salary/i.test(expenseType)) {
    return callback();
  }

  // Extract employee ID from expense type
  let employeeId = null;
  if (expense && (expense.employee_id || expense.emp_id || expense.employeeId)) {
    employeeId = (expense.employee_id || expense.emp_id || expense.employeeId).toString().trim();
  } else {
  ...
  if (!employeeId) {
    console.log("⚠️ Could not extract employee ID for insurance handling");
    return callback();
  }

  const paidMonthYear = paidDate.slice(0, 7);

  // Get employee insurance amount
  db.get(
    `SELECT total_insurance FROM salary_payments WHERE employee_id = ? LIMIT 1`,
    [employeeId],
    (err, empRow) => {
      if (err || !empRow) {
        console.error("⚠️ Could not fetch employee insurance:", err);
        return callback();
      }
      ...
      db.get(
        ...
        (kErr, kittyAcc) => {
          if (kErr || !kittyAcc) {
            console.error("⚠️ KITTY account not found");
            return callback();
          }

          // Debit insurance from current account
          const kittyPrevBal = parseFloat(kittyAcc.balance) || 0;
          const kittyNewBal = kittyPrevBal + insuranceAmount;
          const currentNewBalAfterInsurance = newBalance - insuranceAmount;

          if (currentNewBalAfterInsurance < 0) {
            console.error("⚠️ Insufficient balance for insurance debit");
            return callback();
          }
          ...
          // Debit transaction for insurance
          const debitTxId = `DEB|Regular|Insurance - ${expenseType}|${paidMonthYear}|${timeCode}${seconds}|${rand}`;
          const debitDesc = `Insurance debit for ${expenseType} of month ${paidMonthYear}`;

          // Credit transaction for kitty
          const creditTxId = `CRD|Regular|Insurance - ${expenseType}|${paidMonthYear}|${timeCode}${seconds}|${rand}`;
          const creditDesc = `Insurance credit to KITTY for ${expenseType} of month ${paidMonthYear}`;

          // Update current account balance (debit insurance)
          db.run(
            `UPDATE accounts SET balance = ? WHERE account_id = ?`,
            [currentNewBalAfterInsurance, currentAcc.account_id],
            (updErr) => {
              if (updErr) {
                console.error("⚠️ Failed to debit insurance from current account:", updErr);
                return callback();
              }
              ...
              db.run(
                ...
                (transaction_id, account_number, type, amount, description, previous_balance, updated_balance, created_at)
                VALUES (?, ?, 'Debit', ?, ?, ?, ?, datetime('now'))`,
                [debitTxId, currentAcc.account_number, insuranceAmount, debitDesc, newBalance, currentNewBalAfterInsurance],
                (txErr) => {
                  if (txErr) {
                    console.error("⚠️ Failed to insert debit transaction:", txErr);
                    return callback();
                  }

                  // Update kitty account balance (credit insurance)
                  db.run(
                    `UPDATE accounts SET balance = ? WHERE account_id = ?`,
                    [kittyNewBal, kittyAcc.account_id],
                    (kittyUpdErr) => {
                      if (kittyUpdErr) {
                        console.error("⚠️ Failed to credit insurance to kitty:", kittyUpdErr);
                        return callback();
                      }
                      ...
                      db.run(
                        ...
                        (transaction_id, account_number, type, amount, description, previous_balance, updated_balance, created_at)
                        VALUES (?, ?, 'Credit', ?, ?, ?, ?, datetime('now'))`,
                        [creditTxId, kittyAcc.account_number, insuranceAmount, creditDesc, kittyPrevBal, kittyNewBal],
                        (creditTxErr) => {
                          if (creditTxErr) {
                            console.error("⚠️ Failed to insert credit transaction:", creditTxErr);
                          }
                          callback();
                        }
                      );
...
app.put("/pay-expense", (req, res) => {
  ...
  // 1️⃣ Fetch expense master
  db.get(
    `SELECT * FROM expenses WHERE auto_id = ? LIMIT 1`,
    [expense_id],
    (err, expense) => {
      if (err) return res.status(500).json({ success: false, message: err.message });
      if (!expense)
      ...
      const transactionId = `DEB|${isRegular}|${expenseType}|${paidMonthYear}|${timeCode}${seconds}|${rand}`;
      const description = `Expense paid for ${expenseType} of month ${paidMonthYear}`;
      ...
      db.get(
        ...
        (accErr, currentAcc) => {
          if (accErr)
            return res.status(500).json({ success: false, message: accErr.message });
          if (!currentAcc)
            return res
              .status(500)
          ...
          db.serialize(() => {
            db.run("BEGIN TRANSACTION", (beginErr) => {
              if (beginErr) {
                return res
                  .status(500)
                  .json({ success: false, message: "Transaction start failed" });
              }
                ...
                function (balErr) {
                  ...
                  db.run(
                    ...
                    (txErr) => {
                      ...
                      // targetMonthsForSalary: optional array of month strings (YYYY-MM) to update in monthly_salary_payments; if omitted, uses paidMonthYear
                      const commitAndRespond = (targetMonthsForSalary) => {
                        const monthsToUpdate = targetMonthsForSalary != null
                          ? (Array.isArray(targetMonthsForSalary) ? targetMonthsForSalary : [targetMonthsForSalary])
                          : [paidMonthYear];
                        db.run("COMMIT", () => {
                          // ✅ respond immediately
                          res.json({
                          ...
                          // ✅ defer salary / insurance / kitty updates (after commit)
                          setTimeout(() => {
                            try {
                              // Update monthly_salary_payments for each salary month (e.g. Jan + Feb when both paid)
                              monthsToUpdate.forEach((salaryMonth) => {
                                const m = String(salaryMonth).slice(0, 7);
                                if (!m) return;
                                updateSalaryOnExpensePaid(
                                  expense,
                                  expenseType,
                              ...
                              // Handle insurance debit/credit for salary expenses
                              handleInsuranceForSalaryExpense(
                                expense,
                                expenseType,
                                amount,
                                paid_date,
                                ...
                                newBalance,
                                () => {
                                  console.log("✅ Insurance handling completed");
                                }
                              );
                            } catch (e) {
                              console.error("Post-commit hook failed:", e);
                      ...
                      // ✅ CASE A: Pay existing DB row — use that row's month_year so monthly_salary_payments gets correct month (e.g. Jan salary paid in Feb → update Jan)
                      if (payment_row_id) {
                        db.run(
                          `UPDATE expense_payments
                           SET paid_amount = ?, paid_date = ?, status = 'Paid'
                          ...
                          function (updErr) {
                            ...
                            // Get month_year of the row we just paid (salary month), so monthly_salary_payments is updated for that month
                            db.get(
                              `SELECT month_year FROM expense_payments WHERE id = ?`,
                              [payment_row_id],
                              (rowErr, row) => {
                                const salaryMonth = (row && row.month_year) ? String(row.month_year).slice(0, 7) : paidMonthYear;
                                commitAndRespond(salaryMonth);
                              }
                            );
                          }
                        );
                    ...
                    // ✅ CASE B: Pay ALL rows of this expense in this paid month
else {
  db.run(
    `
    UPDATE expense_payments
    ...
    [
      ...
      paid_date,
      expense_id,
      paidMonthYear,   // carried rows
      paidMonthYear    // auto rows if exist
    ],
    function (updErr) {
      if (updErr) {
        return db.run("ROLLBACK", () =>
      ...
      // 🔥 If no rows existed → create paid row (auto payment fallback)
      if (this.changes === 0) {
        db.run(
          `INSERT INTO expense_payments
          (expense_id, month_year, actual_amount, paid_amount, paid_date, status, due_date)
...
// const sql = `
// WITH params(m) AS (SELECT ?),

// -- =====================================================
// -- 1️⃣ BASE EXPENSES (MASTER DATA)
// -- =====================================================
// expenses_norm AS (
// SELECT
// e.*,
// CAST(strftime('%d', e.due_date) AS INTEGER) AS due_day,
// substr(e.raised_date,1,7) AS start_month
// FROM expenses e
...
// -- =====================================================
// -- 2️⃣ AUTO-GENERATED MONTHLY EXPENSES (REGULAR = YES)
// -- =====================================================
// auto_months AS (
// SELECT
// en.auto_id,
...
// en.amount,
// en.currency,
// en.start_month AS month_year,
// en.due_day,
// date(en.start_month || '-01') AS raised_date,
// CASE
// WHEN en.due_day >
// CAST(strftime('%d',
// date(en.start_month || '-01','+1 month','-1 day')
// ) AS INTEGER)
// THEN date(en.start_month || '-01','+1 month','-1 day')
// ELSE date(en.start_month || '-01','+'||(en.due_day-1)||' day')
// END AS due_date
// FROM expenses_norm en
// WHERE en.regular = 'Yes'
// AND en.start_month <= (SELECT m FROM params)
...
// am.amount,
// am.currency,
// strftime('%Y-%m', date(am.month_year || '-01','+1 month')),
// am.due_day,
// date(strftime('%Y-%m', date(am.month_year || '-01','+1 month')) || '-01'),
// CASE
// WHEN am.due_day >
// CAST(strftime('%d',
// date(
// strftime('%Y-%m', date(am.month_year || '-01','+1 month')) || '-01',
// '+1 month','-1 day'
// )
// ) AS INTEGER)
// THEN date(
...
// END
// FROM auto_months am
// WHERE date(am.month_year || '-01','+1 month')
// <= date((SELECT m FROM params) || '-01')
// ),

// -- =====================================================
// -- 3️⃣ NON-REGULAR EXPENSES (ONE TIME ONLY)
// -- =====================================================
// single_months AS (
// SELECT
// en.auto_id,
...
// en.amount,
// en.currency,
// substr(en.due_date,1,7) AS month_year,
// en.raised_date,
// en.due_date
// FROM expenses_norm en
// WHERE en.regular <> 'Yes'
...
// -- =====================================================
// -- 4️⃣ CARRIED FORWARD / UPDATED EXPENSES
// -- =====================================================
// carried_rows AS (
// SELECT
...
// e.type,
// e.description,
// ep.actual_amount AS amount,
// e.currency,
// ep.month_year,
// e.raised_date,

// -- 🔑 IMPORTANT: dynamic due_date logic
// ep.due_date,
...
// ep.status = 'Paid'
// AND substr(ep.paid_date,1,7) = (SELECT m FROM params)
// )
// OR
// (
// -- ✅ UNPAID / UPDATED → show in DUE month
// ep.status <> 'Paid'
// AND substr(ep.due_date,1,7) = (SELECT m FROM params)
// )
// ),


// -- =====================================================
// -- 5️⃣ AUTO ROWS THAT ARE NOT MOVED
// -- =====================================================
// valid_auto_rows AS (
// SELECT *
// FROM auto_months am
// WHERE am.month_year = (SELECT m FROM params)
// AND NOT EXISTS (
// SELECT 1
// FROM expense_payments ep
// WHERE ep.expense_id = am.auto_id
// AND ep.status <> 'Paid'
// AND substr(ep.due_date,1,7) <> am.month_year
// )
// )
...
// NULL AS paid_amount,
// NULL AS paid_date,
// 'Raised' AS paymentstatus
// FROM valid_auto_rows
...
// 'Raised'
// FROM single_months
// WHERE month_year = (SELECT m FROM params)
...
// res.json(
// rows.map(r => ({
// row_id: r.payment_row_id
// ? `P${r.payment_row_id}` // 🔑 DB-backed row
// : `A${r.auto_id}-${r.month_year}`, // 🔑 auto-generated row

// payment_row_id: r.payment_row_id || null,
...
e.description,
ep.actual_amount AS amount,
ep.actual_amount AS actual_to_pay, -- 🔑 Include actual_to_pay
e.currency,
ep.month_year,
e.raised_date,
...
-- =====================================================
-- 5️⃣ AUTO ROWS THAT ARE NOT MOVED (REGULAR EXPENSES)
-- =====================================================
valid_auto_rows AS (
  SELECT *
  FROM auto_months am
  ...
  AND NOT EXISTS (
    ...
    FROM expense_payments ep
    WHERE ep.expense_id = am.auto_id
    -- 🚫 block ONLY if this expense/month was customized (or already paid) for THIS month_year
    -- (carried-forward rows from other months must NOT hide the auto-generated row)
    AND ep.month_year = am.month_year
  )
)
...
description,
amount,
amount AS actual_to_pay, -- 🔑 Include actual_to_pay
currency,
month_year,
raised_date,
due_date,
...
description,
amount,
actual_to_pay, -- 🔑 Include actual_to_pay from carried_rows
currency,
month_year,
raised_date,
due_date,
...
description: r.description,
amount: r.amount,
actual_to_pay: r.actual_to_pay ?? r.amount, // 🔑 Use actual_to_pay from query or fallback to amount
paid_amount: r.paid_amount ?? 0,
raised_date: r.raised_date,
due_date: r.due_date,
paid_date: r.paid_date || null,
...
const getExpenseFinalAmount = (e) => {
const isPaid =
String(e.status || e.payment_status || "")
.toLowerCase() === "paid";

if (isPaid) {
return Number(e.paid_amount || e.actual_amount || 0);
}
return Number(e.amount || 0);
};

const isInsurance = (type) =>
String(type || "").toLowerCase().startsWith("insurance");
...
// if (isInsurance(r.type)) return; // 🚫 EXCLUDE INSURANCE
...
// actualExpenseItems.reduce(
// (s, e) => s + getExpenseFinalAmount(e),
// 0
// );
...
// const forecastExpenseTotal =
// [...actualExpenseItems, ...forecastExpenseItems].reduce(
// (s, e) => s + getExpenseFinalAmount(e),
// 0
// );
...
app.get("/forecast", async (req, res) => {
  try {
    ...
    // ----------------------------------------------------
    const projects = await runQuery(`
      SELECT projectID, projectName, startDate, endDate, netPayable, invoiceCycle
      FROM Projects
    `);
    ...
    // ----------------------------------------------------
    // 🔑 EXPENSE FETCH (EXACT /getexpenses BEHAVIOR)
    // ----------------------------------------------------
const fetchExpensesForMonth = async (month) => {
  const sql = `
WITH params(m) AS (SELECT ?),
...
-- =====================================================
-- 2️⃣ REGULAR EXPENSES (MONTH-WISE, NO CARRY FORWARD)
-- =====================================================
regular_month AS (
  SELECT
    NULL AS payment_row_id,
    ...
    en.description,
    en.amount,
    en.amount AS actual_amount,
    en.currency,
    (SELECT m FROM params) AS month_year,
    en.raised_date,
    -- dynamic due date in this month
    date((SELECT m FROM params) || '-' || printf('%02d', en.due_day)) AS due_date,
    NULL AS paid_amount,
    NULL AS paid_date,
    'Raised' AS status
  FROM expenses_norm en
  ...
  AND NOT EXISTS (
    ...
    FROM expense_payments ep
    WHERE ep.expense_id = en.auto_id
    AND ep.month_year = (SELECT m FROM params)
  )
),

-- =====================================================
-- 3️⃣ NON-REGULAR EXPENSES (ONE-TIME)
-- =====================================================
single_months AS (
  SELECT
    NULL AS payment_row_id,
  ...
  FROM expenses_norm en
  WHERE en.regular <> 'Yes'
  AND substr(en.due_date,1,7) = (SELECT m FROM params)
  AND NOT EXISTS (
    SELECT 1 FROM expense_payments ep
    WHERE ep.expense_id = en.auto_id
  )
...
-- =====================================================
-- 4️⃣ UPDATED / PAID EXPENSES (USER MOVED)
-- =====================================================
carried_rows AS (
  SELECT
    ep.id AS payment_row_id,
...
-- 5️⃣ FINAL RESULT
-- =====================================================
SELECT * FROM regular_month
UNION ALL
SELECT * FROM single_months
UNION ALL
SELECT * FROM carried_rows
ORDER BY auto_id DESC;
  `;

  return await runQuery(sql, [month]);
};
    ...
    for (const monthKey of monthKeys) {
      ...
      expenseRows.forEach(r => {
        if (String(r.status).toLowerCase() === "paid") {
          actualExpenseItems.push({
            ...
            type: r.type,
            description: r.description,
            amount: Number(r.paid_amount || r.actual_amount || r.amount || 0),
            paid_amount: Number(r.paid_amount || r.actual_amount || 0),
      ...
      const getExpenseFinalAmount = (e) =>
        e.status === "Paid" ? Number(e.paid_amount || 0) : Number(e.amount || 0);

      const actualExpenseTotal =
        actualExpenseItems.reduce((s, e) => s + getExpenseFinalAmount(e), 0);

      const forecastExpenseTotal =
        [...actualExpenseItems, ...forecastExpenseItems]
          .reduce((s, e) => s + getExpenseFinalAmount(e), 0);
      ...
      const actualIncomeTotal =
        actualIncomeItems.reduce((s, a) => s + a.total_with_gst, 0);
      ...
      projects.forEach(p => {
        ...
        if ((sm && monthKey < sm) || (em && monthKey > em)) return;

        const exists = forecastIncomeItems.some(i => i.project_id === p.projectID);
        if (!exists) {
          forecastIncomeItemsCopy.push({
            project_id: p.projectID,
            projectName: p.projectName,
      ...
      const forecastIncomeTotal =
        forecastIncomeItemsCopy.reduce((s, i) => s + Number(i.total_with_gst || 0), 0);
...
app.post("/saveExpensePayment", (req, res) => {
  ...
  const normalizedDueDate = normalize(due_date);
  const normalizedPaidDate = normalize(paid_date);
  status = status || "Raised";
  ...
  // IMPORTANT:
  // - month_year is the ORIGIN month of this expense instance (ex: Jan = "2026-01")
  // - due_date controls which month it SHOWS in the UI (carried forward)
  // So we DO NOT overwrite month_year when due_date changes.
  const originMonthYear = String(month_year).slice(0, 7);
