...
WHERE
  ...
  -- 🔹 NON-REGULAR carry-forward (regular = 'No') UNTIL PAID
  OR (
    am.regular <> 'Yes'
    AND am.month_year < (SELECT m FROM params)
    AND NOT EXISTS (
      SELECT 1
      FROM expense_payments ep4
      WHERE ep4.expense_id = am.auto_id
        AND ep4.status = 'Paid'
        AND substr(ep4.month_year,1,7) <= (SELECT m FROM params)
    )
  )
