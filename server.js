...
// ✅ API to pay salary and record transaction
app.post("/monthlySalary/pay", (req, res) => {
  const { salaryId, account_number } = req.body;

  if (!salaryId || !account_number) {
    return res.status(400).json({ success: false, message: "Salary ID and Account number required" });
  }

  // 1️⃣ Get salary record
  db.get(`SELECT * FROM monthly_salary_payments WHERE id = ?`, [salaryId], (err, salary) => {
    ...
    const amount = salary.actual_to_pay;
    const desc = `Salary payment for ${salary.employee_name} (${salary.month})`;
    const now = new Date().toISOString();

    // 2️⃣ Get account details
    db.get(`SELECT balance FROM accounts WHERE account_number = ?`, [account_number], (err, acc) => {
      if (err) return res.status(500).json({ success: false, message: err.message });
      ...
      db.serialize(() => {
        // 3️⃣ Deduct from account
        db.run(
          `UPDATE accounts SET balance = balance - ? WHERE account_number = ?`,
          [amount, account_number]
        );

        // 4️⃣ Update salary record
        db.run(
          `
        ...
        db.run(
          ...
          (account_number, type, description, amount, related_module, related_id, created_at)
          VALUES (?, 'Outgoing', ?, ?, 'Salary', ?, ?)
          `,
          [account_number, desc, amount, salaryId, now],
          function (err) {
            if (err) {
